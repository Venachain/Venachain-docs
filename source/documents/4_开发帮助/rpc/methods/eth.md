...
-   `data` : 20字节，要检查余额的地址
-   `quantity | TAG` : 区块的块高，或者字符串\"latest\" 、 \"earliest\" 或
    \"pending\"
...
(rpc_methods_eth_getBlockByHash)=
## eth_getBlockByHash
...
```{note}
由于多种原因，例如EVM的机制及节点的性能，估算的数值可能比实际用量大的多。
```
...
所有的属性都是可选的。如果没有指定gas用量上限，venachain将使用挂起块的gas上限。在这种情况下，返回的gas估算量可能不足以执行实际的交易。
...
(rpc_methods_eth_getTransactionByHash)=
...
(rpc_methods_eth_getTransactionReceipt)=
...
(rpc_methods_eth_sendTransaction)=
## eth_sendTransaction
...
```{note}
出于向后兼容的原因，我们接受 "data" 和 "input" 两个参数， "input" 是较新的名称，客户应首选该名称。
```
