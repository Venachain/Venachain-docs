...
-   `object | bool` : 同步状态对象或false。同步对象的结构如下：
    ...
    - currentBlock `quantity` : 节点当前正在同步的区块的块高，同 [venachain_blockNumber](rpc_methods_venachain_blockNumber)
...
(rpc_methods_venachain_blockNumber)=
...
请参考 [venachain_getBlockByHash](rpc_methods_venachain_getBlockByHash) 的返回值。
...
(rpc_methods_venachain_getBlockByHash)=
...
-   `data` : 20字节，合约地址
-   `quantity | TAG` : 区块的块高，或字符串\"latest\"、\"earliest\"
    或\"pending\"
...
-   `quantity | TAG` :
    区块的块高，或字符串\"earliest\"、\"latest\"或\"pending\"
...
请参考 [venachain_getTransactionByHash](rpc_methods_venachain_getTransactionByHash) 的返回值。
...
查阅 [venachain_getTransactionByHash](rpc_methods_venachain_getTransactionByHash) 的返回值
...
(rpc_methods_venachain_getTransactionByHash)=
...
(rpc_methods_venachain_getTransactionReceipt)=
...
(rpc_methods_venachain_sendTransaction)=
...
当创建合约时，在交易生效后，请使用 [venachain_getTransactionReceipt](rpc_methods_venachain_getTransactionReceipt) 调用获取合约地址。
...
当创建合约时，在交易生效后，请使用 [venachain_getTransactionReceipt](rpc_methods_venachain_getTransactionReceipt) 获取合约地址。
...
-   `object` : 交易对象，详情请查看 [venachain_sendTransaction](rpc_methods_venachain_sendTransaction) 
...
(rpc_methods_venachain_newPendingTransactionFilter)=
## venachain_newPendingTransactionFilter \[websocket\] 

在节点中创建一个监听 **pending交易** 产生的过滤器，当产生新的 pending 交易时进行通知。 要检查状态是否发生变化，请调用 [venachain_getFilterChanges](rpc_methods_venachain_getFilterChanges)
...
(rpc_methods_venachain_newBlockFilter)=
## venachain_newBlockFilter \[websocket\] 

在节点中创建一个监听 **新区块** 产生的过滤器，当新区块生成时进行通知。要检查状态是否变化，请调用 [venachain_getFilterChanges](rpc_methods_venachain_getFilterChanges) 
...
(rpc_methods_venachain_newFilter)=
## venachain_newFilter \[websocket\]

基于给定的选项创建一个过滤器对象，当状态发生变化时进行通知。要检查状态是否变化，请调用 [venachain_getFilterChanges](rpc_methods_venachain_getFilterChanges) 关于特定主题【topic】过滤器的说明:主题【topic】是顺序相关的。如果一个交易的日志有主题 `[A, B]` ，那么将被以下的主题过滤器匹配：
...
-   `object` : 过滤器选项对象
    - blockhash `data` : 可选，由 [venachain_getLogs](rpc_methods_venachain_getLogs) 使用，仅从具有此哈希的块返回日志。
...
-   `object` : 过滤器对象，请参考 [venachain_newFilter](rpc_methods_venachain_newFilter) 调用的参数
...
请参考 [venachain_getFilterChanges](rpc_methods_venachain_getFilterChanges) 
...
卸载指定哈希的过滤器。当不再需要监听时，总是需要执行该调用。另外，过滤器如果在一定时间内未接收到 [venachain_getFilterChanges](rpc_methods_venachain_getFilterChanges) 调用会自动超时。
...
(rpc_methods_venachain_getFilterChanges)=
...
1. 使用 [venachain_newBlockFilter](rpc_methods_venachain_newBlockFilter) 创建的过滤器将返回块哈希（32字节），例如 `["0x3454645634534..."]` 。

2. 使用 [venachain_newPendingTransactionFilter](rpc_methods_venachain_newPendingTransactionFilter) 创建的过滤器将返回交易哈希(32字节)，例如 `["0x6345343454645..."]` 。

3. 使用 [venachain_newFilter](rpc_methods_venachain_newFilter) 创建的过滤器，日志对象具有如下参数：
