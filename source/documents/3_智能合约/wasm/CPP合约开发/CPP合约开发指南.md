...
```{warning}
`init()` 方法必须实现为 `public` 类型，这样合约在部署时才能够调用该函数来初始化合约数据。
```
...
`call()` 与 `delegateCall()`
都可以用于调用合约，但是在被调目标合约的视角来看是有区别的，使用
`call()` 时被调合约看到的调用者 `caller` 是发起该调用的合约，而当使用
`delegateCall()` 时，发起调用的合约直接将自身的 `caller`
传递给目标合约。比如在如下的两个例子中，第一种情况下，ContractB看到的
`caller` 是ContractA的地址；而在第二中情况中，ContractB看到的 `caller`
是user的地址。
...
- number()： 当前区块的块高
- timestamp()： 当前区块的时间戳
- blockHash(int64_t number)：给定区块号的哈希值，只支持最近256个区块，且不包含当前区块
